...
const Home = () => {
  ...
  useEffect(() => {
    if (submitted) {
      ...
      input.current.value = "Invitation Sent!";
    }
  }, [submitted]);
  const submit = (e) => {
    e.preventDefault();
  ...
  return (
    <div className="h-screen z-0 flex items-center justify-center">
      <Image
        ...
        width={1920}
        height={1080}
        className="absolute w-full h-full top-0 object-contain"
        priority={true}
      />
      <div className="relative  py-16 flex justify-center items-center flex-col gap-4 w-[50%] bg-[radial-gradient(62.49%_50%_at_50%_50%,#1F1F1F_0%,#0B0B0B_100%)] border-[1px] rounded-[9px] border-[#ffbf00]">
        <h1 className="text-[4.9rem] leading-[0.85] coming-soon text-center">
          COMING
          <br />
          SOON
        </h1>
        <form
          className="flex flex-col gap-3 justify-center items-center w-[50%]"
          onSubmit={(e) => submit(e)}
        >
          <div className={`w-full p-0 m-0 neo-input h-10`}>
            <input
              type="email"
        ...
        <h3 className="text-center font-family2 text-md font-normal text-[#F2E6CF] w-[60%]">
